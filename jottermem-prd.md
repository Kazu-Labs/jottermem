...
- **jottermem-relay** — scaffolded, not deployed. `src/jottermem/relay/` has a working FastAPI app (Google OAuth login/callback, a `DriveStore` backend, and an MCP endpoint gated by the `mcp` SDK's own token-verifier/resource-server mechanism) — boot-tested locally with dummy credentials. It cannot actually run anywhere until you provide a Google Cloud OAuth app and pick a host; see `src/jottermem/relay/README.md` for exactly what's needed and what's deliberately simplified (self-issued bearer tokens rather than a full OAuth 2.1 authorization server, single SQLite token store) ahead of a real launch.
- **Search** — basic keyword search only, per the V1 scope line above, not semantic.
- **App / browse-and-edit UI** — done, as `jottermem-app`: a zero-dependency local web view (stdlib `http.server`, bound to `127.0.0.1` only) to see topics/fact counts, add a fact, and hand-edit a topic's raw markdown, wired into the setup wizard's closing instructions. Local-folder and Drive-for-Desktop-synced folders both work today since it just reads/writes files on disk; it doesn't yet talk to the relay's Drive-API backend directly (editing a Drive-backed folder still works if Drive for Desktop has it mounted locally).
